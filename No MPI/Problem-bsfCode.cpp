...
void PC_bsf_Init(bool* success) {
	...
	* success = MTX__Load_Problem();
	...
	if (PD_meq > 0) {
		int rankEq;
		Matrix_Rank(PD_edgeBasis_v, PD_meq, PP_EPS_ZERO, &rankEq);
		if (PD_meq != rankEq) {
...
void PC_bsf_MapF(PT_bsf_mapElem_T* mapElem, PT_bsf_reduceElem_T* reduceElem, int* mapSuccess) {
	...
	for (int coi = 0; coi < batch_size; coi++) {
		...
		for (int i = PD_meq_basis; i < PD_n - 1; i++)
			PD_edgeBasis_v[i] = PD_edgeNeHyperplanes[i - PD_meq_basis];

		int rank;
		Matrix_Rank(PD_edgeBasis_v, PD_n - 1, PP_EPS_ZERO, &rank);
		if (rank < PD_n - 1)
			continue;
