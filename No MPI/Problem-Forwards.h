...
//====================== Private Functions ===========================
unsigned long long PF_BinomialCoefficient(int n, int k);
void	PF_CalculateNumberOfEdges(int n, int mh, int* me);
void	PF_MakeEdgeList(int* edgeCodeList, int me);
...
void	PF_Print_Number_of_edges(PT_vector_T x);
void	PF_PseudoprojectionOnEdge(PT_vector_T v, PT_vector_T w, double eps, int* success);
void	PF_TWIDDLE(int* x, int* y, int* z, int p[PP_N + 2], bool* done);
void	PF_TWIDDLE_CodeToSubset(int code, int a[PP_MM], int c[PP_N - 1], int n, int m);
void	PF_TWIDDLE_Make_p(int p[PP_MM + 2], int n, int m);
//====================== Shared Functions ===========================
void	SF_Conversion();
